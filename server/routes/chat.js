...
router.post('/', CheckUser, async (req, res) => {
    ...
    try {
        if(prompt_model=="sonnet"){
            res.write('Agent-1: Response: \n \n')
            const agent1 = await anthropic.messages.create({
                model: "claude-3-sonnet-20240229",
                max_tokens: 4000,
                temperature: 0.1,
                system: system_msg[0].text + (option=="notes")?" Don't write anything in Chief Complaint section of clinical notes":""+(option=="letters")?" Also needs to make sure the letter is tailored to the end user (either the patient or dentist) and is more concise":"",
                messages: [
                    {
                        "role": "user",
                        "content": [
            ...
            const agent2 = await anthropic.messages.create({
                ...
                max_tokens: 4000,
                temperature: 0.2,
                system: `Here’s the response for task ${prompt}: ${res_agent1}`,
                messages:[
                    {
                        "role": "user",
                        "content": [
                        {
                            "type": "text",
                            "text": "Check the response carefully for correctness and identify any clinical inconsistent or missing information from a clinical standpointand. give constructive criticism and analyse for how to improve it."
                        }
                        ]
                    }
                ],
                ...
                messages:[
                    {
                        ...
                        {
                            "type": "text",
                            "text": res_agent2 +" Use this criticism to improve results and rewrite the response in detailed manner."+ (option=="notes")?" Also remove data in Chief Complaint section of clinical notes":"" + (option=="letters")?" Also needs to make sure the letter is tailored to the end user (either the patient or dentist) and is more concise":""
                        }
                        ]
                    }
                ],
